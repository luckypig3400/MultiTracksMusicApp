...
### 請Gemini 3 Pro預覽版在歌詞播放界面、播放介面歌名區塊、設定界面新增許多使用小功能
#### 0 test
`上傳本專案所有程式碼文件到Code->FileExplorer內，排除vscode和git設定以及 需求.txt`
你好我已經把所有本專案的程式碼都上傳到File Explorer裡面，然後裡面有2個是我不想要的檔案分別是index.tsx與metadata.json，
請你幫我把這2個檔案刪除，並且告訴我上傳到File Explorer裡面的每一份程式碼分別有多少行，讓我可以確保目前你和我看到的檔案是一模一樣的
**測試結果:**不行沒辦法，Gemini沒辦法刪除那兩個檔案，但是我上傳的檔案可以手動刪除，而且它可以讀取

#### 1st 也不算錯啦XD 歌詞的offset變成調整對齊秒數了XDDD
你好，我已經把所有本專案的程式碼都上傳到File Explorer裡面，但是index.tsx與metadata.json這不是我的檔案，請無視他們
我希望在歌詞播放界面新增可以調整當前歌詞顯示位置的offset輸入框(只能+-3，用拉桿就好不用輸入框)，以及點一下歌名區塊可以複製這首歌的名字，
並在下方彈出懸浮視窗"已複製歌名(Copied title)"，然後在左/右邊點2下歌名區塊跳轉秒數的設定，提供在設定界面可以更改，輸入框設為number即時保存不用按鈕

請你仔細瀏覽我傳的所有程式(app.js,index.html,lyrics.js,playlist.js,setting.js)碼，並仔細思考後再開始寫程式碼，
請完整輸出你所有有修改的程式碼 !!!請勿刪除任何一行註解，也不要壓縮程式碼!!! 超重要!!! 謝謝~

#### 2nd 播放中歌詞的offset變成調整對齊秒數了XDD 不需要啦(不然還是保留好了XD)
謝謝，做得很棒! 大部分的功能都改好了，只是我說的歌詞offset的意思其實是只播放中歌詞在歌詞界面的的顯示位置，
讓他可以調整播放中高亮歌詞的位置，以電腦來看目前這樣的設定是蠻好的，但是放到手機上看會覺得高亮狀態的歌詞，
顯示的位置有點低所以我才說offset輸入框(只能+-3，用拉桿就好不用輸入框)，他的名字叫做播放中歌詞顯示位置調整，不用輸入框然後要整數，
+1代表往上一段歌詞的位置，以此類推，這個調整高亮歌詞顯示位置的拉桿可以放到下一列，不然手機介面看起來會太擠
另外我覺得你做的歌詞同步秒數調整也很棒，請把他的名稱改為歌詞同步秒數調整，預設為0，然後針對每首歌的歌詞調整的值，
也會存在設定里，拉桿移掉改用輸入框的方式，一樣用成number我記得可以設定+-，這次就不用有正負三的限制了
...
#### 4th Not Bad
測試後發現Android Chrome下載後開啟離線網頁一樣無法順利運作
主要的無法運作的問題在於選擇檔案的那個彈出視窗，無論是
選擇資料夾 (電腦版): 還是 選擇多個檔案 (手機/平板版):
他們的選擇檔案都會無法點選，甚至連關閉按鈕也沒有作用，
不確定是不是Android Chrome下載離線網頁本身的異常還是我們程式碼的異常，
請你針對這個議題詳細搜尋，解決這個問題
然後你修改的icons 感覺跟原本的相比比較不明顯了，尤其是那個隨機和重複播放按鈕
他們在啟用狀態下那個黃色非常不明顯，我在想是不是因為icons線條變太細導致的
最後是我認為不需要那個 選擇多個檔案 (手機/平板版):
畢竟載入資料夾更方便，多選檔案在手機/平板上也有點怪怪的
